...
start(CoreArgs, SegsNodes = [_Nodes | _], RepSettings) when is_list(_Nodes) ->
   ...
   CoreSettings = {kv_core, CoreArgs},
   {_, Segs} = {_, [Seg1 | _]} = lists:foldl(
      fun(Nodes, {Start, Acc}) ->
            End = Start+SegWidth,
            NewAcc = [
               repobj:new({Start, End}, CoreSettings, Nodes, RepSettings) | Acc
            ],
            {End, NewAcc}
      end,
      {0, []},
   ...
   [ spawn(N, fun() -> kvstracker:start(Segs) end) || N <- lists:flatten(SegsNodes) ].
...
put(K, V) ->
   ...
   Segment = route(Key, kvstracker:view()),
   repobj:cmd(Segment, {Key, {put, Value}}).
...
split(K) ->
   ...
   if
      ...
      ?ELSE ->
         ...
         kvstracker:addConfs([Seg1, Seg2]),

         % remove the keys out of range in both segments
         repobj:cmd(Seg1, rm_out_of_range),
         repobj:cmd(Seg2, rm_out_of_range),
...
merge(K) ->
   ...
   if
      ...
      ?ELSE ->
         ...
         utils:multicall(Seg1, {update_conf, NewSeg}),
...
set_succ(Seg, Succ) ->
   repobj:cmd(Seg, {sequencer, {set_succ, Succ}}).

% Delete the segment's successor
del_succ(Seg) ->
   repobj:cmd(Seg, {sequencer, del_succ}).
